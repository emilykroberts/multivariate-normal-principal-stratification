...
run_sim_unequalarms = function(SIM, ST, X, trt){
  ...
  {holdmu= matrix(rep(0, 4*SIM), 4, SIM)
    ...
    slope= array(0, c((SIM), 1)); int= array(0, c((SIM), 1))
  ...
  while(sim<=SIM){
    ...
    for(i in 1:n){
      ...
      if(dim((mu[i, c(2, 4)] + (SIG[c(2, 4), c(1, 3)] %*% ginv(SIG[c(1, 3), c(1, 3)]))))[2] == dim( (as.matrix(t(ST[i, c(1, 3)] - mu[i, c(1, 3)]))))[1]){
